...
def main():
    ...
    if user_name != "":
        ...
        # Select mode
        page = st.selectbox(
            "What do you want to do?", ["Create My Wishlist", "Buy For Others"]
        )
        ...
        if page == "Create My Wishlist":
            ...
            edited_selection_df = st.data_editor(
                user_wishlist[["Remove", "Name", "Item", "Description", "Link"]],
                hide_index=True,
            )

            if st.button("Remove Items"):
                df = df.drop(
                    user_wishlist[edited_selection_df["Remove"]].index.to_list()
                )
                CONN.update(data=df)
                st.success("Item submitted successfully!")
            ...
            selected_person = st.selectbox(
                "Select a person to view their list:", others_names
            )
            ...
            edited_selection_df = st.data_editor(
                selected_person_wishlist[
                    ["Select", "Name", "Item", "Description", "Link", "Claimed By"]
                ],
                hide_index=True,
                key=1,
            )
            ...
            if col1.button("Claim Items"):
                selections = selected_person_wishlist[
                    edited_selection_df["Select"]
                ].index.to_list()
                for selection in selections:
                    ...
                    if row["Claimed By"] is None or pd.isna(row["Claimed By"]):
                        ...
                        st.success(
                            f"Successfully claimed {row['Item']} for {row['Name']}."
                        )
                    elif row["Claimed By"] == user_name:
                        st.error("You've already claimed this item.")
                    else:
                        st.error(
                            f"Someone else has already claimed {row['Item']} for {row['Name']}."
                        )
                CONN.update(data=df)
            ...
            if col2.button("Unclaim Items", key=2):
                ...
                for selection in selections:
                    ...
                    if row["Claimed By"] == user_name:
                        ...
                        st.success(
                            f"Successfully unclaimed {row['Item']} for {row['Name']}."
                        )
                    else:
                        st.error(
                            f"You hadn't claimed buying {row['Item']} for {row['Name']}."
                        )
                CONN.update(data=df)
